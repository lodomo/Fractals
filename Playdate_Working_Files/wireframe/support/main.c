...
int project(double observer_distance, double halfangle_degrees) {
    ...
    for (i = 0; i < N; i++) {
        Yplot[i] =
            ((Y[i] * SCREEN_DIM) / (2 * (observer_distance + Z[i]) * tan(halfangle_degrees)) + HALF_SCREEN_DIM);
        Xplot[i] =
            ((X[i] * SCREEN_DIM) / (2 * (observer_distance + Z[i]) * tan(halfangle_degrees)) + HALF_SCREEN_DIM);
    }
...
int print_object() {
    ...
    for (i = 0; i < N; i = i + 2) {
        printf("(%lf, %lf)   (%lf, %lf)\n", Xplot[i], Yplot[i], Xplot[i + 1],
               Yplot[i + 1]);
    }
    return i;
}

int save_line(double xs, double ys, double zs, double xe, double ye,
              double ze) {
...
int test_pyramid() {
    G_init_graphics(SCREEN_DIM, SCREEN_DIM);
    G_rgb(0, 0, 0);
    G_clear();
    G_rgb(0, 1, 0);
